...
def build_payload() -> dict:
    ...
    msa_features = pd.merge(msa_features, df_oer[["msa_code", "OER"]], on="msa_code", how="left")
    ...
    factor_df["Implied_Value"] = (
        factor_df["Median_Gross_Rent"] * 12 * (1 - factor_df["OER"])
    ...
    scored["Capital_Index"] = scored[["Cap Spread", "Diff_Effective_Rate"]].mean(axis=1)
    ...
    raw = scored["Index_Score_Raw"]
    iqr = raw.quantile(0.75) - raw.quantile(0.25)
    if iqr == 0:
        scored["Investment_Score"] = 50.0
    else:
        z_robust = (raw - raw.median()) / iqr
        scored["Investment_Score"] = 100 / (1 + np.exp(-1.6 * z_robust))
    ...
    for _, row in merged.iterrows():
        ...
        record = {
            ...
            "Market_Tightness": float(max(0.0, 1.0 - vacancy_rate)),
            "Value_Potential": implied_value,
            "Diff_Effective_Rate": float(row["Diff_Effective_Rate"]),
            "Cap Spread": float(row["Cap Spread"]),
